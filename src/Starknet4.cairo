...
// This is a bit challenging for Joe and Jill, Liz prepared an outline for 
// how contract should work, can you help Jill and Joe write it?.
...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            assert(get_caller_address() == owner, 'NOT_THE_OWNER');
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let current_stock = self.inventory.read(product);
            assert(current_stock >= quantity, 'INSUFFICIENT_STOCK');
